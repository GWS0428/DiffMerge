...
class tomePipeline(StableDiffusionXLPipeline):
    ...
    def opt_token(self, latents: torch.Tensor, t, stoken, prompt_anchor, iter_num=3):
        ...
        # NOTE(wsgwak): Add auxiliary scheduler to avoid chaning the main scheduler.
        # NOTE(wsgwak): The original schduler state revert is not correct.. Check it.
        if type(self.scheduler).__name__ == "DDIMScheduler":
            aux_scheduler = type(self.scheduler).from_config(self.scheduler.config)
            aux_scheduler.set_timesteps(self._num_timesteps, device=stoken.device)
        elif type(self.scheduler).__name__ == "EulerDiscreteScheduler":
            # EulerDiscreteScheduler is a default scheduler for TOME pipeline.
            # However, it has internal state other than timesteps. 
            # TODO(wsgwak): Check if the original code is correct.
            pass
        else:
            raise ValueError(
                f"Scheduler {type(self.scheduler).__name__} is not supported for TOME pipeline."
            )  
            
        # TODO(wsgwak): extra_step_kwargs should be set correctlly. generator should be given
        extra_step_kwargs = self.prepare_extra_step_kwargs(None, 0.0)

        # NOTE(wsgwak): check if cfg eq is right
        with torch.no_grad():
            # NOTE(wsgwak): We should use the new noise_pred_token using optimized stoken (after while loop)
            # noise_pred_opt_stoken = self.unet(
            #     latents,
            #     t,
            #     encoder_hidden_states=stoken, # Use the stoken_input derived from the optimized stoken
            #     timestep_cond=self.timestep_cond,
            #     cross_attention_kwargs=self.cross_attention_kwargs,
            #     added_cond_kwargs=self.added_cond_kwargs2,
            # ).sample
            ...
            # NOTE(wsgwak): This original approach require to use the main scheduler and restore the internal state.
