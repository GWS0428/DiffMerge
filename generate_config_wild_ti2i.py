...
# wild-ti2i (PnP)
print("Loading benchmark data for wild-ti2i...")
...
yaml_file_path = f"{file_dir}/datasets/{dataset_name}/wild-ti2i-real.yaml"
output_dir = f"{file_dir}/configs/{dataset_name}"
dataset_dir = f"{file_dir}/datasets/{dataset_name}/data"
...
for entry in yaml_data:
    ...
    img_name_pure = img_name_base.replace("_", " ")  # Get the first part before the underscore
    ...
    for i, prompt in enumerate(target_prompts):
        ...
        current_config["img_path"] = os.path.join(dataset_dir, os.path.basename(init_img))
        current_config["prompt"] = prompt
        current_config["src_prompt"] = f"a photo of a {img_name_pure}"
        current_config["output_path"] = os.path.join(current_config["output_path"], dataset_name)
